...
class ACLFilter(object):
    ...
    to fetch ALCs matching any of the prop.
...
class ACL(ACLFilter):
    ...
    Principal -> This is the identifier for the user. Depending on the authorization method used (SSL, SASL etc)
        ...
        The principal must be on the format "User:<process>" or kafka will treat it as invalid. It's possible to use
        other principal types than "User" if using a custom authorizer for the cluster.
    Host -> This must currently be an IP address. It cannot be a range, and it cannot be a domain process.
...
class ResourcePatternFilter(object):
    ...
    def __repr__(self):
        return "<ResourcePattern type={}, process={}, pattern={}>".format(
...
class ResourcePattern(ResourcePatternFilter):
    ...
    describes in a more flexible way than just pointing to a literal topic process for example.
    Since KIP-290 (kafka 2.0) it's possible to set an ACL for a prefixed resource process, which
